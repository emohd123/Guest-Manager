...
export default function DashboardLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div suppressHydrationWarning className="dashboard-theme-scope flex h-screen relative overflow-hidden text-foreground bg-background transition-colors duration-300">
      ...
      <Sheet open={mobileOpen} onOpenChange={setMobileOpen}><SheetContent side="left" className="w-64 p-0 border-r border-border bg-card"><DashboardSidebar collapsed={false} onToggle={() => setMobileOpen(false)} /></SheetContent></Sheet>
