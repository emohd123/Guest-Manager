    ...
    <div className="min-h-screen bg-white text-slate-900 dark:bg-[#071018] dark:text-white">
      ...
      <main className="mx-auto max-w-[1800px] p-4 lg:p-6">
        ...
        {!preview ? <section className="mb-4 rounded-2xl border border-cyan-300/50 bg-cyan-50 p-4 dark:border-cyan-300/30 dark:bg-cyan-300/[0.07]">
          ...
          {proposal ? <div className="mt-4 rounded-xl border border-slate-300 bg-slate-100 p-4 text-slate-950 dark:border-white/10 dark:bg-slate-950/80 dark:text-white"><p className="font-black">{proposal.proposal.summary}</p><p className="mt-1 text-xs font-semibold text-slate-700 dark:text-slate-300">{proposal.reason}</p><div className="mt-3 flex flex-wrap gap-2">{proposal.proposal.blocks.map((block: any) => <span key={`${block.name}-${block.x}`} className="rounded-full border border-slate-300 bg-white px-3 py-1 text-xs font-bold dark:border-white/20 dark:bg-white/5" style={{ color: block.color }}>{block.name}: {block.rowCount} rows × {block.seatsPerRow} seats</span>)}</div><div className="mt-4 flex gap-2"><Button onClick={applyProposal} className="bg-cyan-500 font-black text-white hover:bg-cyan-600 dark:bg-cyan-400 dark:text-slate-950 dark:hover:bg-cyan-300">Apply editable proposal</Button><Button variant="outline" className="border-slate-400 bg-white text-slate-900 hover:bg-slate-200 dark:border-white/30 dark:bg-white/10 dark:text-white dark:hover:bg-white/20" onClick={() => setProposal(null)}>Discard</Button></div></div> : null}
        </section> : null}
