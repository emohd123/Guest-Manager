      ...
      <header className="border-b border-white/10 bg-[#09141d] px-5 py-4">
        <div className="mx-auto flex max-w-[1800px] flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <Link href={`/dashboard/events/${eventId}`} className="grid h-10 w-10 place-items-center rounded-xl border border-white/20 bg-white/5 text-white hover:bg-white/10"><ChevronLeft className="h-5 w-5" /></Link>
            <div><p className="text-xs font-black uppercase tracking-[.2em] text-cyan-300">Seating designer</p><h1 className="text-xl font-black text-white">Build your hall map</h1></div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <label className="flex h-10 items-center gap-2 rounded-xl border border-white/20 bg-white/5 px-3 text-xs font-bold text-white"><input type="checkbox" checked={enabled} onChange={(event) => setEnabled(event.target.checked)} /> Map active</label>
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
            <section className="min-w-0 overflow-hidden rounded-2xl border border-white/10 bg-[#0a151e]">
              <div className="flex flex-wrap items-center gap-2 border-b border-white/10 p-3">
                ...
                <Button size="sm" variant="outline" className="border-white/30 bg-white/10 text-white hover:bg-white/20 disabled:bg-white/5 disabled:text-white/35" onClick={generateChairs} disabled={selection.section === null}><Armchair className="mr-2 h-4 w-4" />Add rows & chairs</Button>
                <Button size="sm" variant="ghost" className="text-cyan-200 hover:bg-white/10 hover:text-white disabled:text-white/30" disabled={uploading} onClick={() => fileRef.current?.click()}><ImagePlus className="mr-2 h-4 w-4" />{uploading ? "Uploading…" : floorPlanUrl ? "Replace background" : "Background"}</Button>
                ...
                <Button size="icon" variant="ghost" className="text-white hover:bg-white/10 hover:text-white disabled:text-white/25" onClick={undo} disabled={!undoStack.length}><Undo2 className="h-4 w-4" /></Button>
                <Button size="icon" variant="ghost" className="text-white hover:bg-white/10 hover:text-white disabled:text-white/25" onClick={redo} disabled={!redoStack.length}><Redo2 className="h-4 w-4" /></Button>
