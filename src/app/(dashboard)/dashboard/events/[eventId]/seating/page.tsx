    ...
    <div className="min-h-screen bg-white text-slate-900 dark:bg-[#071018] dark:text-white">
      ...
      <main className="mx-auto max-w-[1800px] p-4 lg:p-6">
        ...
        {!preview ? <section className="mb-4 rounded-2xl border border-cyan-300/50 bg-cyan-50 p-4 dark:border-cyan-300/30 dark:bg-cyan-300/[0.07]">
          <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center"><div><p className="flex items-center gap-2 text-sm font-black text-cyan-700 dark:text-cyan-200"><Sparkles className="h-4 w-4" />Start with your floor plan</p><p className="mt-1 text-xs leading-5 text-slate-600 dark:text-white/65">Upload a PNG, JPG or WebP, then create an editable seating proposal. With OPENAI_API_KEY configured, iTicket uses vision analysis; otherwise it creates a clearly labelled assisted grid proposal for you to review.</p></div><Button onClick={analyseFloorPlan} disabled={!floorPlanUrl || analysing || lockedInventory} className="shrink-0 bg-cyan-500 font-black text-white hover:bg-cyan-600 dark:bg-cyan-400 dark:text-slate-950 dark:hover:bg-cyan-300">{analysing ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}{analysing ? "Analysing…" : "Analyse floor plan"}</Button></div>
