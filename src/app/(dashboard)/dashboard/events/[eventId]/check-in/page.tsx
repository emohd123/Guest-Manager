  ...
  if (!checkinV2Enabled) {
    return (
      <div className="p-20 text-center space-y-6">
        ...
        <h1 className="text-2xl font-black text-white italic uppercase tracking-tighter">Check-in Disabled</h1>
        <p className="text-white/20 text-[10px] font-bold uppercase tracking-widest max-w-sm mx-auto">
          Check-in reporting is currently unavailable. Enable the new check-in experience to view arrivals here.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-12 pb-20 px-2">
      ...
      <div className="grid gap-6 sm:grid-cols-3">
        <StatCard label="Checked In" value={totals.checkedIn} icon={CheckCircle} color="text-green-400" bg="bg-green-500/10" delay={0} />
        <StatCard label="Checked Out" value={totals.checkedOut} icon={LogOut} color="text-blue-400" bg="bg-blue-500/10" delay={0.1} />
        <StatCard label="Invalid Scans" value={totals.invalid} icon={XCircle} color="text-red-400" bg="bg-red-500/10" delay={0.2} />
      </div>
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-center justify-between">
        <div className="relative flex-1 max-w-xl group">
          ...
          <Input
            ...
            placeholder="Search guests, tickets, or scan data..."
            value={search}
      ...
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="rounded-[40px] bg-white/5 border border-white/10 overflow-hidden shadow-2xl">
        <div className="grid grid-cols-12 gap-6 px-10 py-6 border-b border-white/5 bg-white/2">
          ...
          <div className="col-span-2 text-[10px] font-black uppercase tracking-widest text-white/30">Action</div>
          <div className="col-span-3 text-[10px] font-black uppercase tracking-widest text-white/30">Guest</div>
          <div className="col-span-2 text-[10px] font-black uppercase tracking-widest text-white/30">Device</div>
          <div className="col-span-2 text-[10px] font-black uppercase tracking-widest text-white/30 text-right">Result</div>
        </div>
        ...
        <div className="divide-y divide-white/5">
          {isLoading ? (
            <div className="p-20 text-center space-y-4">
               ...
               <p className="text-[10px] font-black text-white/20 uppercase tracking-widest">Loading arrivals...</p>
            </div>
          ) : rows.length === 0 ? (
            <div className="p-24 text-center space-y-6">
              <Target className="h-16 w-16 text-white/5 mx-auto" />
              <div className="space-y-1">
                <h4 className="text-xl font-black text-white italic uppercase tracking-tighter">No Arrival Activity</h4>
                <p className="text-white/20 text-[10px] font-bold uppercase tracking-widest">No scans match the current filter</p>
              </div>
            </div>
          ) : (
            <AnimatePresence mode="popLayout">
              {rows.map((row, i) => {
                const attendee = row.attendeeName || `${row.guestFirstName ?? ""} ${row.guestLastName ?? ""}`.trim() || "Unknown Guest";
                    ...
                    <div className="col-span-3">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                           ...
                           <p className="text-[9px] font-bold text-white/20 uppercase tracking-widest font-mono italic">{row.barcode ?? "NO-TICKET"}</p>
                        </div>
                      </div>
                    </div>
                    <div className="col-span-2">
                       <p className="text-[10px] font-black text-white italic tracking-tight uppercase leading-none mb-1">{row.deviceName || "WEB SCANNER"}</p>
