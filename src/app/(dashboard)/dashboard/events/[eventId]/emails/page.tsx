    ...
    <div className="space-y-12 pb-20 px-2">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
        <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
          <h1 className="text-4xl font-black text-foreground italic tracking-tighter uppercase leading-none">Emails</h1>
       ...
       <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="space-y-8">
          <div className="flex gap-4 items-center">
             <div className="relative group flex-1 max-w-md">
                ...
                <Input placeholder="Filter recipient email..." className="h-12 rounded-2xl border-border bg-card/80 pl-14 text-[10px] font-black italic uppercase tracking-widest text-foreground placeholder:text-muted-foreground focus:ring-primary transition-all" />
             </div>
       ...
       <Dialog open={!!viewingEmail} onOpenChange={(o) => !o && setViewingEmail(null)}>
        <DialogContent className="max-w-xl rounded-[40px] border border-border bg-popover p-12 shadow-2xl backdrop-blur-3xl dark:border-white/10 dark:bg-slate-950">
          <DialogHeader className="mb-10">
            <p className="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-2 italic">Email Details</p>
            <DialogTitle className="text-3xl font-black text-foreground italic uppercase tracking-tighter dark:text-white">Email Summary</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={!!activityEmail} onOpenChange={(o) => !o && setActivityEmail(null)}>
        <DialogContent className="max-w-xl rounded-[40px] border border-border bg-popover p-12 shadow-2xl backdrop-blur-3xl dark:border-white/10 dark:bg-slate-950">
          <DialogHeader className="mb-10">
             <p className="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-2 italic">Delivery Timeline</p>
            <DialogTitle className="text-3xl font-black text-foreground italic uppercase tracking-tighter dark:text-white">Email Activity</DialogTitle>
          </DialogHeader>
          <div className="relative ml-6 space-y-10 border-l-2 border-border pl-10 dark:border-white/10">
             {[
                { title: "Email Created", time: activityEmail?.createdAt, icon: Zap, color: "text-white/40", bgColor: "bg-white/10" },
                { title: activityEmail?.state === 'Delivered' ? 'Email Delivered' : 'Sending...', time: activityEmail?.updatedAt, icon: ShieldCheck, color: "text-green-500", bgColor: "bg-green-500/10" },
                activityEmail?.openCount > 0 && { title: "Opened", time: `${activityEmail.openCount} Opens`, icon: Target, color: "text-primary", bgColor: "bg-primary/10" },
                activityEmail?.clickCount > 0 && { title: "Link Clicks", time: `${activityEmail.clickCount} Clicks`, icon: Activity, color: "text-orange-500", bgColor: "bg-orange-500/10" }
      ...
      <Dialog open={!!resendingEmail} onOpenChange={(o) => !o && setResendingEmail(null)}>
        <DialogContent className="max-w-xl rounded-[40px] border border-border bg-popover p-12 shadow-2xl backdrop-blur-3xl dark:border-white/10 dark:bg-slate-950">
          <DialogHeader className="mb-10">
             <p className="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-2 italic">Resend Email</p>
             <DialogTitle className="text-3xl font-black text-foreground italic uppercase tracking-tighter dark:text-white">Confirm Resend</DialogTitle>
            <DialogDescription className="mt-4 text-[10px] font-bold uppercase leading-relaxed tracking-widest text-muted-foreground dark:text-white/20">
               Resend &quot;{resendingEmail?.type}&quot; to {resendingEmail?.emailAddress}?
            </DialogDescription>
          </DialogHeader>
