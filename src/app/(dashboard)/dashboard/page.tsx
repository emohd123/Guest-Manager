  ...
  return (
    <div className="space-y-12 pb-20 px-2">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        >
          <h1 className="text-4xl font-black text-white italic tracking-tighter uppercase">Operations</h1>
      ...
      <div className="grid gap-10 lg:grid-cols-3">
        ...
        <div className="space-y-10 lg:col-span-2">
          ...
          >
            <div className="relative z-10 space-y-8">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  <h3 className="text-2xl font-black text-white italic leading-none mb-2">Command Center Setup</h3>
                  <p className="text-white/30 text-[10px] font-bold uppercase tracking-widest">Initialize your operational workspace</p>
                </div>
                ...
                {setupSteps.map((step, i) => (
                  <Link key={step.number} href={step.href}>
                    ...
                    >
                      ...
                      <div>
                        <h4 className={cn("font-black italic text-sm mb-1 uppercase tracking-tight", step.done ? "text-green-400" : "text-white")}>
                          {step.title}
                        </h4>
          ...
          >
            <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/2 backdrop-blur-xl">
              <div>
                <h3 className="text-xl font-black text-white italic leading-none mb-2">Deployment Schedule</h3>
                <p className="text-white/30 text-[10px] font-bold uppercase tracking-widest">Upcoming active operations</p>
              </div>
              <Link href="/dashboard/events">
                <Button variant="ghost" className="h-10 px-4 rounded-xl text-white/20 hover:text-white hover:bg-white/5 font-black text-[10px] uppercase tracking-widest flex gap-2 group">
              ...
              {upcomingLoading ? (
                ...
                <div className="p-16 text-center space-y-6">
                  ...
                  <div className="space-y-2">
                    <h4 className="text-lg font-black text-white italic uppercase tracking-tighter">No Active Deployments</h4>
                    <p className="text-white/20 text-[10px] font-bold uppercase tracking-widest">Awaiting command input</p>
                  </div>
                  <Link href="/dashboard/events/new">
                ...
                <div className="space-y-3">
                  {upcomingEvents.events.map((event, i) => (
                    <Link
                      ...
                      className="group flex items-center justify-between p-6 rounded-[24px] bg-white/3 border border-white/5 hover:bg-white/8 transition-all"
                    >
                      <div className="flex items-center gap-6">
                         ...
                         <div>
                           <p className="font-black text-white text-lg italic tracking-tight leading-none mb-1">{event.title}</p>
                      ...
                      <div className="flex items-center gap-8">
                        <div className="text-right hidden sm:block">
                           <p className="text-[9px] font-black text-white/10 uppercase tracking-widest mb-1">Status</p>
                           <Badge className="bg-white/10 text-white/60 border-none font-black text-[9px] uppercase tracking-widest px-3 italic">
                             {event.status}
                           </Badge>
                        </div>
        ...
        <div className="space-y-10">
          ...
          <motion.div
            ...
            className="rounded-[40px] bg-white/5 border border-white/10 p-8 space-y-6"
          >
            <h3 className="text-lg font-black text-white italic uppercase tracking-tighter ml-2">Flash Actions</h3>
            <div className="grid gap-4">
              {[
                ...
                <Link key={action.label} href={action.href}>
                  <Button variant="outline" className="w-full h-16 justify-between gap-4 px-6 rounded-2xl bg-white/3 border-white/5 hover:bg-white/8 hover:border-primary/50 transition-all group">
                    <div className="flex items-center gap-4">
                      <div className={cn("h-10 w-10 flex items-center justify-center rounded-xl", action.bg)}>
                        <action.icon className={cn("h-5 w-5", action.color)} />
                      </div>
                      <span className="font-black italic text-sm text-white/80">{action.label}</span>
                    </div>
                    <Plus className="h-4 w-4 text-white/20 group-hover:text-primary transition-colors" />
                  </Button>
                </Link>
              ))}
            </div>
          ...
          <motion.div
             ...
             className="rounded-[40px] bg-white/5 border border-white/10 p-8 space-y-8"
          >
            <div className="flex items-center justify-between ml-2">
              <h3 className="text-lg font-black text-white italic uppercase tracking-tighter">Live Feed</h3>
              ...
              <div className="space-y-6">
                <div className="flex gap-4 items-start group">
                   ...
                   <div className="space-y-1">
                      <p className="text-sm font-black text-white italic">{totalCheckIns} UNITS VERIFIED</p>
                      <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest">Global success rate</p>
                   </div>
                </div>
              </div>
            ) : (
              <div className="py-10 text-center space-y-4">
                <Clock className="h-10 w-10 text-white/5 mx-auto" />
          ...
          >
            <div className="relative z-10 space-y-6">
               <div className="flex items-center justify-between">
                 <h3 className="text-xl font-black text-white italic tracking-tighter uppercase leading-none">Power</h3>
                 <Badge className="bg-primary text-white font-black text-[9px] uppercase border-none px-3">BASIC</Badge>
               </div>
               
               <div className="space-y-4">
                 <div className="flex justify-between items-end">
                    <p className="text-[10px] font-black text-white/40 uppercase tracking-widest">Resource Usage</p>
                    <p className="text-xl font-black text-white italic leading-none">{totalCheckIns} <span className="text-[10px] text-white/20">/ 50</span></p>
                 </div>
               ...
               <Link href="/dashboard/settings" className="block">
                  <Button className="h-14 w-full rounded-2xl bg-white/5 border border-white/10 text-white hover:bg-white/10 font-black italic uppercase tracking-widest transition-all">
                    Upgrade Protocol
                  </Button>
               </Link>
            </div>
