  ...
  return (
    <div className="space-y-12 pb-20">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 px-2">
        <div>
          <h1 className="text-4xl font-black text-foreground dark:text-white italic tracking-tighter">Finance</h1>
          <p className="text-muted-foreground dark:text-white/40 font-bold uppercase tracking-[0.2em] text-[10px] mt-2">
            Real-time transaction tracking
          </p>
        </div>
        <Button className="theme-ghost-surface h-14 px-8 rounded-2xl font-black text-base transition-all hover:-translate-y-1 flex gap-3">
          ...
          >
            <div className="relative z-10 flex flex-col h-full">
              <div className="flex items-center justify-between mb-6">
                ...
                <Badge className="rounded-full border-none bg-green-500/20 px-2 py-0.5 font-black text-[10px] text-green-400">
                  {stat.trend}
                </Badge>
              </div>
              <p className="mb-1 text-[10px] font-black uppercase tracking-widest text-muted-foreground dark:text-white/30">{stat.label}</p>
              <h2 className="mb-4 text-3xl font-black text-foreground dark:text-white italic tracking-tight">{stat.value}</h2>
              <p className="mt-auto text-[10px] font-bold uppercase tracking-tighter text-muted-foreground/70 dark:text-white/10">{stat.sub}</p>
            </div>
            {/* Visual Flare */}
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4 px-2">
          <h2 className="text-xl font-black text-foreground dark:text-white italic">Transaction History</h2>
          <div className="h-px flex-1 bg-border dark:bg-white/5" />
        </div>

        <div className="px-2 flex flex-col gap-6 sm:flex-row">
          <div className="relative group flex-1">
            <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground transition-colors group-focus-within:text-primary dark:text-white/30" />
            <Input
              placeholder="Order Number, Name or Email..."
              className="theme-input h-14 rounded-2xl pl-12 transition-all"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
          </div>
          <Select value={status} onValueChange={setStatus}>
            <SelectTrigger className="theme-input h-14 w-[200px] rounded-2xl font-bold focus:ring-primary">
        ...
        {isLoading ? (
          <div className="theme-panel p-20 text-center">
            <div className="h-12 w-12 animate-spin rounded-full border-4 border-primary border-t-transparent mx-auto mb-4" />
            <p className="text-muted-foreground dark:text-white/40 font-black uppercase tracking-widest text-xs">Syncing Ledger...</p>
          </div>
        ) : !data?.orders?.length ? (
          <div className="theme-panel p-24 text-center">
            <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-3xl bg-muted dark:bg-white/3">
              <Package className="h-10 w-10 text-muted-foreground/30 dark:text-white/10" />
            </div>
            <h3 className="mb-2 text-xl font-black text-foreground dark:text-white">No Transactions</h3>
            <p className="mx-auto max-w-xs text-sm text-muted-foreground dark:text-white/30">
          ...
          <div className="theme-panel overflow-hidden backdrop-blur-3xl">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/40 text-left text-muted-foreground dark:border-white/5 dark:bg-white/5 dark:text-white/40">
                ...
                <tbody className="divide-y divide-border dark:divide-white/5">
                  {data.orders.map((order) => (
                    <tr key={order.id} className="group transition-all hover:bg-muted/40 dark:hover:bg-white/5">
                      <td className="py-6 px-8">
                        <span className="font-mono font-black text-primary text-xs tracking-tighter">{order.orderNumber}</span>
                      </td>
                      <td className="py-6 px-8">
                        <div className="flex flex-col">
                          <p className="mb-1 font-bold text-foreground dark:text-white leading-none">{order.name || "Anonymous"}</p>
                          <p className="text-[10px] font-medium text-muted-foreground dark:text-white/40">{order.email}</p>
                        </div>
                      </td>
                      <td className="py-6 px-8">
                      ...
                      <td className="py-3 px-8 text-muted-foreground dark:text-white/40 font-bold uppercase tracking-tighter text-[10px]">
      ...
      <motion.div
        ...
        viewport={{ once: true }}
        className="mx-2 flex flex-col items-center justify-between gap-8 rounded-[40px] border border-[#635BFF]/30 bg-linear-to-br from-[#635BFF]/20 to-transparent p-10 md:flex-row"
      >
        <div className="flex items-center gap-8">
          ...
          <div>
            <h3 className="mb-2 text-2xl font-black text-foreground dark:text-white italic tracking-tight">Stripe Gateway</h3>
