...
export default function TicketsPage() {
  ...
  const quickStats = [
    { label: "Ticket Types", value: isLoading ? "-" : totalTypes.toString(), icon: Ticket, sub: "Defined tiers", delay: 0 },
    { label: "Templates", value: "4", icon: Palette, sub: "Ready-made layouts", delay: 0.1 },
    { label: "Issued Tickets", value: isLoading ? "-" : totalSold.toString(), icon: Hash, sub: "Tickets delivered", delay: 0.2 },
    { label: "Live Scans", value: "0", icon: ScanLine, sub: "Check-in activity", delay: 0.3 },
  ];
  ...
  return (
    <div className="space-y-12 pb-20 px-2">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        >
          <h1 className="text-4xl font-black text-foreground dark:text-white italic tracking-tighter uppercase">Ticket Center</h1>
          <p className="text-muted-foreground dark:text-white/40 font-bold uppercase tracking-[0.2em] text-[10px] mt-2">
            Ticket catalog, templates, and scanning
          </p>
        </motion.div>
              ...
              {isLoading ? (
                ...
                <div className="p-20 text-center space-y-6">
                  ...
                  <div className="space-y-2">
                    <h4 className="text-lg font-black text-foreground dark:text-white italic uppercase tracking-tighter">No Ticket Types Yet</h4>
                    <p className="text-muted-foreground/70 dark:text-white/20 text-[10px] font-bold uppercase tracking-widest">Create ticket tiers inside an event to get started</p>
                  </div>
                  <Link href="/dashboard/events">
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
                            ...
                            <span className="text-[10px] font-black text-muted-foreground/70 dark:text-white/20 uppercase tracking-widest flex items-center gap-2">
                              ...
                              {(tt.price ?? 0) === 0 ? "FREE" : `$${((tt.price ?? 0) / 100).toFixed(2)}`}
                            </span>
                            <span className="text-[10px] font-black text-muted-foreground/70 dark:text-white/20 uppercase tracking-widest flex items-center gap-2">
                              <Hash className="h-3 w-3" />
                              {tt.quantitySold ?? 0} / {tt.quantityTotal ?? "INF"} SOLD
                            </span>
                          </div>
                        </div>
                      </div>
                ...
                >
                  <div className={cn("m-4 h-48 rounded-[32px] p-6 flex flex-col justify-between border relative overflow-hidden", template.style)}>
                    <div className="flex items-center justify-between relative z-10">
                      <span className="text-[8px] font-black uppercase tracking-[0.3em] opacity-50 italic">TEMPLATE PREVIEW</span>
                      <Zap className="h-4 w-4 opacity-30" />
                    </div>
                    <div className="relative z-10">
                      <p className="text-sm font-black italic tracking-tighter uppercase line-clamp-1">Sample Guest</p>
        ...
        <TabsContent value="scanner" className="mt-0 outline-none">
          ...
          >
            <div className="relative z-10 space-y-6">
              ...
              <div className="space-y-3">
                <h3 className="text-3xl font-black text-foreground dark:text-white italic tracking-tighter uppercase leading-none">Scanner Setup</h3>
                <p className="max-w-md mx-auto text-[10px] font-bold text-muted-foreground dark:text-white/30 uppercase tracking-[0.2em] leading-relaxed">
                  Connect check-in devices or open the camera scanner for live ticket validation.
                </p>
              </div>
