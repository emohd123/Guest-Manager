...
export default function PrivateEventsDashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-7">
      ...
      {isLoading ? <div className="grid gap-4 md:grid-cols-2"><Skeleton className="h-52" /><Skeleton className="h-52" /></div> : conferences.length === 0 ? <section className="rounded-3xl border border-dashed p-10 text-center"><KeyRound className="mx-auto h-8 w-8 text-cyan-500" /><h2 className="mt-4 text-xl font-black">No private conferences yet</h2><p className="mt-2 text-sm text-muted-foreground">Create a private conference here to manage its access and attendee portal separately.</p></section> : <section className="grid gap-4 md:grid-cols-2">{conferences.map((event) => <article key={event.id} className="rounded-3xl border bg-card p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-widest text-cyan-500">{event.status}</p><h2 className="mt-2 text-xl font-black">{event.title}</h2><p className="mt-2 text-sm text-muted-foreground">{new Intl.DateTimeFormat(undefined, { dateStyle: "medium", timeStyle: "short" }).format(new Date(event.startsAt))}</p></div><span className="rounded-xl bg-cyan-500/10 px-3 py-2 font-mono text-sm font-black text-cyan-600">{event.visitorCode ?? "SET CODE"}</span></div><div className="mt-6 grid gap-2 sm:grid-cols-2"><Button size="sm" asChild><Link href={`/dashboard/private-events/${event.id}`}><LayoutPanelTop className="mr-1.5 h-4 w-4" /> Manage conference</Link></Button><Button size="sm" variant="outline" asChild><Link href={`/dashboard/events/${event.id}/guests`}><Users className="mr-1.5 h-4 w-4" /> Attendees</Link></Button></div></article>)}</section>}
    </div>
  );
}
