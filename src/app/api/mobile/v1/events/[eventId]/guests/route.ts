...
import { authenticateMobileRequest, getIncrementalGuests } from "@/server/services/checkin";
...
export async function GET(
  ...
  try {
    ...
    const guests = await getIncrementalGuests(db, {
      eventId,
      companyId,
    ...
    if (error instanceof z.ZodError) {
      return jsonError("Invalid query params", 400, "validation_failed");
    }
    ...
    return jsonError(error instanceof Error ? error.message : "Unable to fetch guests", 400, "guest_sync_failed");
  }
}
