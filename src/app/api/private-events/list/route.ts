...
function isPrivateAccessEnabled(settings: unknown) {
  const publicPage = settingsObject(settings).publicPage as
    | { enabled?: boolean }
    | undefined;
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      .from("events")
      .select(
        "id,venue_id,title,short_description,description,cover_image_url,starts_at,ends_at,timezone,visitor_code,settings",
      )
    ...
    const conferences = ((data ?? []) as ConferenceRow[]).filter((event) =>
      isPrivateAccessEnabled(event.settings),
    );
    const venueIds = [
      ...new Set(conferences.map((event) => event.venue_id).filter(Boolean)),
    ] as string[];
    ...
    const venueNames = new Map(
      ((venuesData ?? []) as VenueRow[]).map((venue) => [venue.id, venue.name]),
    );
      ...
      conferences: conferences.map((event) => {
        ...
        return {
          ...
          description:
            ...
            event.description ??
            "Private conference details are available after access is verified.",
          ...
          venueName:
            (typeof publicPage.venueName === "string" &&
