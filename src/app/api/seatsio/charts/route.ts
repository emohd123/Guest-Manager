...
export async function GET(request: NextRequest) {
  ...
  const response = await fetch("https://api-eu.seatsio.net/charts?limit=20", {
  ...
  const charts = Array.isArray(payload) ? payload : Array.isArray(payload.items) ? payload.items : [];
