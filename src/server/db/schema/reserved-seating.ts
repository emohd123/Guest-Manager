...
export const seatSections = pgTable("seat_sections", {
  id: uuid("id").primaryKey().defaultRandom(),
  planId: uuid("plan_id").references(() => seatingPlans.id, {onDelete: "cascade"}).notNull(),
...
export const seatRows = pgTable("seat_rows", {
  id: uuid("id").primaryKey().defaultRandom(),
  sectionId: uuid("section_id").references(() => seatSections.id, {onDelete: "cascade"}).notNull(),
  label: varchar("label", {length: 30}).notNull(),
  price: integer("price"),
  sortOrder: integer("sort_order").default(0).notNull(),
}, (table) => [uniqueIndex("seat_rows_section_label_idx").on(table.sectionId, table.label)]);

export const reservedSeats = pgTable("reserved_seats", {
  id: uuid("id").primaryKey().defaultRandom(),
  rowId: uuid("row_id").references(() => seatRows.id, {onDelete: "cascade"}).notNull(),
  label: varchar("label", {length: 30}).notNull(),
  price: integer("price"),
  x: integer("x"),
  y: integer("y"),
...
}, (table) => [uniqueIndex("reserved_seats_row_label_idx").on(table.rowId, table.label)]);

export const seatHolds = pgTable("seat_holds", {
  id: uuid("id").primaryKey().defaultRandom(),
  seatId: uuid("seat_id").references(() => reservedSeats.id, {onDelete: "cascade"}).notNull().unique(),
