...
import { mergeSeatsIoChartKey, readSeatsIoChartKey } from "@/lib/seatsio";
...
export function SeatsIoDashboard({ eventId }: { eventId: string }) {
  ...
  useEffect(() => { if (event) setChartKey(readSeatsIoChartKey(event.settings)); }, [event]);
  useEffect(() => {
  ...
  const saveChart = async () => {
    if (!chartKey) return;
    setSavingChart(true);
    try { await updateEvent.mutateAsync({ id: eventId, settings: mergeSeatsIoChartKey(event.settings, chartKey) }); }
    finally { setSavingChart(false); }
  };

  return <main className="min-h-[70vh] bg-slate-50 p-4 text-slate-950 dark:bg-slate-950 dark:text-white md:p-8"><div className="mx-auto max-w-6xl space-y-6"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.2em] text-cyan-600">Seats.io seating</p><h1 className="text-3xl font-black">{event.title}</h1><p className="mt-2 max-w-2xl text-sm text-slate-600 dark:text-white/60">Manage this event’s chart in Seats.io. The customer map uses the chart selected below.</p></div><div className="flex flex-wrap gap-2"><Link href={`/dashboard/events/${eventId}`} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-black text-slate-800 hover:bg-slate-50 dark:border-white/20 dark:bg-slate-900 dark:text-white">Back to event</Link>{canOpenPublicPage && publicEventHref ? <Link href={`${publicEventHref}?openSeats=1`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-cyan-300 bg-cyan-50 px-4 py-3 text-sm font-black text-cyan-900 hover:bg-cyan-100 dark:border-cyan-500/40 dark:bg-cyan-950/40 dark:text-cyan-100">Open customer event <ExternalLink className="h-4 w-4" /></Link> : null}{chartKey ? <Link href={`https://app.seats.io/workspace/${WORKSPACE_KEY}/charts/${chartKey}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-cyan-600 px-4 py-3 text-sm font-black text-white hover:bg-cyan-700">Open selected chart <ExternalLink className="h-4 w-4" /></Link> : null}</div></div><section className="rounded-2xl border border-cyan-200 bg-cyan-50 p-5 dark:border-cyan-500/30 dark:bg-cyan-950/30"><div className="flex flex-wrap items-end gap-3"><label className="min-w-[280px] flex-1 text-sm font-black text-slate-900 dark:text-white">Seats.io chart for this event<select value={chartKey ?? ""} onChange={(e) => setChartKey(e.target.value || null)} className="mt-2 w-full rounded-xl border border-cyan-300 bg-white px-3 py-3 font-semibold text-slate-900 dark:border-cyan-500/40 dark:bg-slate-900 dark:text-white"><option value="">Select a chart…</option>{charts.map((chart) => <option key={chart.key} value={chart.key}>{chart.name} ({chart.key})</option>)}</select></label><button type="button" onClick={() => void saveChart()} disabled={savingChart || !chartKey} className="rounded-xl bg-cyan-600 px-5 py-3 text-sm font-black text-white hover:bg-cyan-700 disabled:cursor-not-allowed disabled:opacity-50">{savingChart ? "Saving…" : "Use this chart"}</button></div>{chartsError ? <p className="mt-3 text-sm font-semibold text-rose-700 dark:text-rose-300">{chartsError}. Confirm your Seats.io secret key and refresh.</p> : null}<p className="mt-3 text-xs text-cyan-900 dark:text-cyan-100">Choose a chart from your Seats.io workspace, then save. Customer booking will use this event’s chart and will not reuse another event’s chart.</p></section><section className="grid gap-4 md:grid-cols-[1fr_280px]"><div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-slate-900"><div className="mb-3 flex items-center gap-2 font-black"><LayoutGrid className="h-5 w-5 text-cyan-500" /> Customer preview</div><p className="mb-4 text-sm text-slate-600 dark:text-white/60">Preview is scoped to <strong>{event.title}</strong>. Click “Choose seats” to open its map.</p><SeatsIoChart eventId={eventId} workspaceKey={WORKSPACE_KEY} chartKey={chartKey ?? undefined} autoOpen={false} /></div><aside className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-white/10 dark:bg-slate-900"><Settings2 className="h-6 w-6 text-cyan-500" /><h2 className="mt-3 font-black">How to update</h2><ol className="mt-3 list-decimal space-y-3 pl-5 text-sm text-slate-600 dark:text-white/65"><li>Select the chart you created in Seats.io.</li><li>Save it for this event.</li><li>Edit sections, rows, seats and category prices in Seats.io.</li><li>Refresh the customer event to see changes.</li></ol><p className="mt-5 rounded-xl bg-cyan-50 p-3 text-xs text-cyan-950 dark:bg-cyan-950/40 dark:text-cyan-100">The older native map data remains preserved as a fallback for events without a selected Seats.io chart.</p></aside></section></div></main>;
}
