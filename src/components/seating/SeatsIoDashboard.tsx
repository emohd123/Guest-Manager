...
export function SeatsIoDashboard({ eventId }: { eventId: string }) {
  ...
  const removeChart = async () => {
    if (!chartKey) return;
    if (!window.confirm("Remove the seating map from this event? Customers will see the regular ticket options instead. Your Seats.io chart will not be deleted.")) return;
    setSavingChart(true);
    setSaveError(null);
    try {
