...
type Props = { eventId: string; eventKey?: string; workspaceKey: string; chartKey?: string; basePrice?: number; currency?: string; ticketTypeId?: string; ticketTypeName?: string; autoOpen?: boolean; returnTo?: string; companySlug?: string; eventSlug?: string; onSelection?: (ids: string[]) => void };
export function SeatsIoChart({ eventId, eventKey, workspaceKey, chartKey, basePrice = 0, currency = "EGP", ticketTypeId, ticketTypeName = "Reserved seat", autoOpen = false, returnTo, companySlug, eventSlug, onSelection }: Props) {
  ...
  <button type="button" disabled={!selected.length} onClick={() => { const safeReturnTo = returnTo || (companySlug && eventSlug ? `/e/${companySlug}/${eventSlug}?openSeats=1#tickets` : ""); if (safeReturnTo) { try { window.sessionStorage.setItem("iticket.seatReturnTo", safeReturnTo); } catch {} } const items = [{ ticketTypeId: ticketTypeId || "", name: ticketTypeName, price: basePrice, currency, quantity: selected.length }]; const params = new URLSearchParams({ event: eventSlug || "", company: companySlug || "", items: JSON.stringify(items), seats: selected.join(","), seatsIo: "1" }); if (safeReturnTo) params.set("returnTo", safeReturnTo); router.push(`/checkout/tickets?${params.toString()}`); }} className="rounded-lg bg-zinc-900 px-4 py-2 font-black text-white disabled:cursor-not-allowed disabled:opacity-40 sm:px-7 sm:py-3">Next</button>
      </div>, document.body) : null}
    </div>
  </>;
}
