...
export function SeatsIoChart({ eventId, workspaceKey, basePrice = 0, currency = "EGP", autoOpen = false, onSelection }: Props) {
  ...
  return <>
    ...
    <div className={fullScreen ? "fixed inset-0 z-[200] flex flex-col bg-white" : "w-full"} role={fullScreen ? "dialog" : undefined} aria-modal={fullScreen ? true : undefined} aria-label={fullScreen ? "Choose seats" : "Choose seats"}>
      {fullScreen && typeof document !== "undefined" ? createPortal(<div style={{ position: "fixed", top: 0, left: 0, right: 0 }} className="fixed inset-x-0 top-0 z-[2147483647] flex h-14 items-center border-b border-zinc-200 bg-white px-3 shadow-sm sm:h-16 sm:px-4">
      ...
      {fullScreen && typeof document !== "undefined" ? createPortal(<div style={{ position: "fixed", bottom: 0, left: 0, right: 0 }} className="fixed inset-x-0 bottom-0 z-[2147483647] grid h-16 grid-cols-[auto_1fr_auto] items-center gap-2 border-t border-zinc-200 bg-white px-3 pb-[env(safe-area-inset-bottom)] text-xs shadow-[0_-4px_16px_rgba(0,0,0,.12)] sm:h-20 sm:gap-4 sm:px-5 sm:text-sm">
