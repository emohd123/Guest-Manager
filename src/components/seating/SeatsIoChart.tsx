...
type Props = { eventId: string; eventKey?: string; workspaceKey: string; chartKey?: string; basePrice?: number; currency?: string; autoOpen?: boolean; returnTo?: string; companySlug?: string; eventSlug?: string; onSelection?: (ids: string[]) => void };
export function SeatsIoChart({ eventId, eventKey, workspaceKey, chartKey, basePrice = 0, currency = "EGP", autoOpen = false, returnTo, companySlug, eventSlug, onSelection }: Props) {
  ...
  return <>
    ...
    <button type="button" disabled={!selected.length} onClick={() => { const params = new URLSearchParams({ event: eventId, seats: selected.join(","), subtotal: (selected.length * basePrice).toFixed(2), currency }); const safeReturnTo = returnTo || (companySlug && eventSlug ? `/e/${companySlug}/${eventSlug}?openSeats=1#tickets` : ""); if (safeReturnTo) { params.set("returnTo", safeReturnTo); try { window.sessionStorage.setItem("iticket.seatReturnTo", safeReturnTo); } catch {} } if (companySlug) params.set("company", companySlug); if (eventSlug) params.set("eventSlug", eventSlug); router.push(`/checkout/seats?${params.toString()}`); }} className="rounded-lg bg-zinc-900 px-4 py-2 font-black text-white disabled:cursor-not-allowed disabled:opacity-40 sm:px-7 sm:py-3">Next</button>
      </div>, document.body) : null}
    </div>
  </>;
}
