...
export function SeatsIoChart({ eventId, workspaceKey, onSelection }: Props) {
  ...
  return <>
    ...
    <div className={fullScreen ? "fixed inset-0 z-[200] flex flex-col bg-white" : "w-full"} role={fullScreen ? "dialog" : undefined} aria-modal={fullScreen ? true : undefined} aria-label={fullScreen ? "Choose seats" : "Choose seats"}>
      {fullScreen ? <div className="relative z-[220] flex items-center justify-between border-b border-zinc-200 bg-white px-4 py-3 shadow-sm">
      ...
      {fullScreen ? <div className="relative z-[220] flex items-center justify-between gap-4 border-t border-zinc-200 bg-white px-5 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] text-sm shadow-[0_-4px_16px_rgba(0,0,0,.08)]">
