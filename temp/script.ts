...
async function main() {
  ...
  const res = await db.execute(sql`
    SELECT id, company_id, first_name, last_name FROM guests WHERE email = ${email} AND event_id = ${eventId} LIMIT 1
  `);
  if (res.length === 0) {
    console.log('Guest not found');
    process.exit(1);
  }
  const guest = res[0] as any;
  console.log('Found guest:', guest.id);

  const tktRes = await db.execute(sql`
    SELECT id FROM tickets WHERE guest_id = ${guest.id} LIMIT 1
  `);
  if (tktRes.length > 0) {
  ...
  const typesRes = await db.execute(sql`
    SELECT id FROM ticket_types WHERE event_id = ${eventId} LIMIT 1
  `);
  if (typesRes.length === 0) {
      console.log('No ticket types found');
      process.exit(1);
  }
  const ticketType = typesRes[0] as any;
